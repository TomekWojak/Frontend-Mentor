...
const addErrorState = (box) => {
    box.classList.add("error");
};
const removeErrorState = (box) => {
    box.classList.remove("error");
};
const showErrorTxt = (txt, season, isEmpty, value, isShorter) => {
...
const handleInputs = () => {
    ...
    inputs.forEach((input, index, inputs) => {
        ...
        const season = input.previousElementSibling?.textContent.toLowerCase() ?? "";
        let isEmpty = false;
        let isValid = false;
        let isShorter = false;
        if (!parent)
            return;
        if (!(errorTxt instanceof HTMLElement))
            return;
        ...
        switch (season) {
            ...
            case "month":
                ...
                const mainErrorTxt = document.querySelector(".error-txt-main");
                if (!mainErrorTxt)
                    return;
...
const checkIfShorter = (month, inputs) => {
    const dayInputValue = inputs.find((input) => input.placeholder === "DD")?.value;
    const yearInputValue = inputs.find((input) => input.placeholder === "YYYY")?.value;
    if (!dayInputValue || !yearInputValue)
        return false;
